...
LLMs have a limited context length. This means that you can only pass a limited amount of data to each call.
...
3. We find the chunk that is semantically the closest to the user query
4. This chunk is passed to the LLM call

llm-functions-ts tries it best to find the best chunk to pass to the LLM.
