import { Execution, Trace } from '../llm';
...
export const Response: React.FC<ResponseProps> = ({ response }) => {
  ...
  return (
    <div className="text-sm">
      {json.success ? (
        <Inspector expandLevel={10} table={false} data={json} />
      ) : (
        response
      )}
    </div>
...
export type TraceProps = { data: Execution<any>[] };
  ...
  return (
    <div className=" w-full flex">
        ...
        <div className="flex flex-col h-full overflow-auto">
          <div className="flex flex-col h-full relative">
            <div className="flex flex-col gap-2 p-4 divide-y divide-neutral-800">
              {data.map(({ trace, id, inputs, finalResponse }, i) => {
                return (
                          ...
                          return (
                            <div
                              ...
                              className={`flex rounded flex-col bg-neutral-800 roudned-xl p-2 cursor-pointer ${
                                selectedAction?.[0] === i &&
                                selectedAction?.[1] === tIdx
                                  ? 'outline outline-sky-500'
                                  : ''
                              }`}
                              onClick={() => setSelectedAction([i, tIdx])}
                            >
      ...
      <div className="flex flex-1 w-fit flex-col gap-2 border-l border-neutral-800 max-w-[500px] p-4 h-full overflow-auto">
        ...
        {selectedAction !== undefined &&
          renderAction(data[selectedAction[0]].trace[selectedAction[1]])}
      </div>
    </div>
  );
};
